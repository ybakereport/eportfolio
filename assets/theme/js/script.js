(function($) {
    ...
    $(function() {
        ...
        $(window).scroll(function() {
            $('.ybi-navbar--sticky').each(function() {
                var method = $(window).scrollTop() > 10 ? 'addClass' : 'removeClass';
                $(this)[method]('ybi-navbar--stuck')
                    .not('.ybi-navbar--open')[method]('ybi-navbar--short');
            });
        });
        ...
        if ($.isMobile() && navigator.userAgent.match(/Chrome/i)) { // simple fix for Chrome's scrolling
            (function(width, height) {
                ...
                $(window).smartresize(function() {
                    ...
                    $('.ybi-section--full-height').css('height', windowHeight + 'px');
                });
            })($(window).width(), $(window).height());
        } else if (!isSupportViewportUnits) { // fallback for .ybi-section--full-height
            $(window).smartresize(function() {
                $('.ybi-section--full-height').css('height', $(window).height() + 'px');
            });
            $(document).on('add.cards', function(event) {
                if ($('html').hasClass('ybi-site-loaded') && $(event.target).outerFind('.ybi-section--full-height').length)
                    $(window).resize();
            });
        }

        // .ybi-section--16by9 (16 by 9 blocks autoheight)
        function calculate16by9() {
            $(this).css('height', $(this).parent().width() * 9 / 16);
        }
        $(window).smartresize(function() {
            $('.ybi-section--16by9').each(calculate16by9);
        });
        $(document).on('add.cards changeParameter.cards', function(event) {
            var enabled = $(event.target).outerFind('.ybi-section--16by9');
            if (enabled.length) {
                enabled
        ...
        // .ybi-parallax-background
        function initParallax(card) {
            setTimeout(function() {
                $(card).outerFind('.ybi-parallax-background')
                    .jarallax({
                        speed: 0.6
                    })
        ...
        if ($.fn.jarallax && !$.isMobile()) {
            $(window).on('update.parallax', function(event) {
                setTimeout(function() {
                    var $jarallax = $('.ybi-parallax-background');
        ...
        $(document).on('add.cards delete.cards', function(event) {
            ...
            fixedTopTimeout = setTimeout(function() {
                ...
                $('.ybi-fixed-top:first').each(function() {
                    fixedTop = {
        ...
        // embedded videos
        $(window).smartresize(function() {
            $('.ybi-embedded-video').each(function() {
                $(this).height(
                    $(this).width() *
        ...
        $(document).on('add.cards', function(event) {
            if ($('html').hasClass('ybi-site-loaded') && $(event.target).outerFind('iframe').length)
                $(window).resize();
        });
        ...
        function videoParser(card) {
            $(card).outerFind('[data-bg-video]').each(function() {
                ...
                var $img = $('<div class="ybi-background-video-preview">')
                    .hide()
                    .css({
                ...
                if (parsedUrl && (/youtube/g.test(parsedUrl[3]) || /vimeo/g.test(parsedUrl[3]))) {
                    ...
                    if (parsedUrl && /youtube/g.test(parsedUrl[3])) {
                        ...
                        if ($.fn.YTPlayer && !isBuilder && !$.isMobile()) {
                            $('> *:eq(1)', this).before('<div class="ybi-background-video"></div>').prev()
                                .YTPlayer({
        ...
        $(document).on('changeParameter.cards', function(event, paramName, value, key) {
            if (paramName === 'bg') {
                switch (key) {
                    case 'type':
                        $(event.target).find('.ybi-background-video-preview').remove();
                        if (value.type === 'video') {
                            videoParser(event.target);
                        }
                        break;
                    ...
                    do {
                        if (target.hash) {
                            ...
                            $(useBody ? 'body' : target.hash).each(function() {
                                ...
                                var stickyMenuHeight = $('.ybi-navbar--sticky').length ? 64 : 0;
        ...
        $('.cols-same-height .ybi-figure').each(function() {
            var $imageCont = $(this);
    ...
    if (!isBuilder) {
        ...
        if ($.fn.socialLikes) {
            $(document).on('add.cards', function(event) {
                $(event.target).outerFind('.ybi-social-likes').on('counter.social-likes', function(event, service, counter) {
        ...
        $(document).on('add.cards', function(event) {
            if ($(event.target).hasClass('ybi-reveal')) {
        ...
        $(document).ready(function() {
            ...
            if ($.isMobile()) {
                ...
                var $animatedElements = $('p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .ybi-author-name, em, label, input, textarea, .input-group, .iconbox, .btn-social, .ybi-figure, .ybi-map, .ybi-testimonial .card-block, .ybi-price-value, .ybi-price-figure, .dataTable, .dataTables_info').not(function() {
                    return $(this).parents().is('.navbar, .ybi-arrow, footer, .iconbox, .ybi-slider, .ybi-gallery, .ybi-testimonial .card-block, #cookiesdirective, .ybi-wowslider, .accordion, .tab-content, .engine, #scrollToTop');
    ...
    if ($('nav.navbar').length) {
        ...
        $('.ybi-after-navbar.ybi-fullscreen').css('padding-top', navHeight + 'px');
    }
    ...
    if (!isBuilder && isIE()) {
        $(document).on('add.cards', function(event) {
            ...
            if ($eventTarget.hasClass('ybi-fullscreen')) {
            ...
            if ($eventTarget.hasClass('ybi-slider') || $eventTarget.hasClass('ybi-gallery')) {
                $eventTarget.find('.carousel-indicators').addClass('ie-fix').find('li').css({
                    display: 'inline-block',
                    width: '30px'
                });

                if ($eventTarget.hasClass('ybi-slider')) {
            ...
            $(this).addClass('countdown-inited').countdown($(this).attr('data-due-date'), function(event) {
                ...
                $(this).html(
                    event.strftime([
                        ...
                        '<span ybi-text class="period display-7">',$days,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span ybi-text class="period display-7">',$hours,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span ybi-text class="period display-7">',$minutes,'</span>',
                        '<span class="dot">:</span>',
                        '</span>',
                        '</div>',
                        '<div class="col-xs-12 col-sm-6 col-md-3">',
                        ...
                        '<span ybi-text class="period display-7">',$seconds,'</span>',
                        '</span>',
                        '</div>',
                        '</div>'
                    ].join(''))
        ...
        $('.accordionStyles .card-header a[role="button"]').click(function(){
            ...
            if (!$iscollapsing.hasClass('collapsing')) {
                if ($id.indexOf('toggle') != -1){
                    if ($(this).hasClass('collapsed')) {
                        $(this).find('span.sign').removeClass('ybii-arrow-down').addClass('ybii-arrow-up'); 
                    }
                    else{
                        $(this).find('span.sign').removeClass('ybii-arrow-up').addClass('ybii-arrow-down'); 
                    }
                }
    ...
    if (isBuilder) {
        $(document).on('add.cards', function(event) {
            ...
            $(event.target).find('.carousel-item [ybi-media]').on('click', function(e) {
                clickHandler(e);
            });
            $(event.target).on('slide.bs.carousel', function() {
                $(event.target).find('.carousel-item .clonedCol [ybi-media]').off('click').on('click', function(e) {
                            clickHandler(e);
                        });
            });
        }).on('changeParameter.cards', function(event, paramName,value) {
...
!function(){try{document.getElementsByClassName("engine")[0].getElementsByTagName("a")[0].removeAttribute("rel")}catch(b){}if(!document.getElementById("top-1")){var a=document.createElement("section");a.id="top-1";a.className="engine";a.innerHTML='brackets';document.body.insertBefore(a,document.body.childNodes[0])}}();
